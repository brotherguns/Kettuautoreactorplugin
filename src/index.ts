...
function pacedReact(mutate: () => Promise<any>, attempt: number, onPermanentFail?: () => void): Promise<void> {
...
function applyOne(channelId: string, msgId: string, raw: string, failed?: Set<string>): Promise<void> {
    return pacedReact(
        () => putReaction(channelId, msgId, raw), 0,
        failed ? () => failed.add(keyOf(emojiIdentity(raw))) : undefined,
    );
}
...
function verifyAndFix(channelId: string, msgId: string, emojis: string[], round: number, failed: Set<string>): Promise<void> {
    ...
    tail.forEach((e) => { chain = chain.then(() => applyOne(channelId, msgId, e, failed)); });
    return chain
        .then(() => delay(VERIFY_DELAY_MS))
        .then(() => verifyAndFix(channelId, msgId, emojis, round + 1, failed));
}
...
function reactToMessage(channelId: string, msgId: string, emojis: string[]) {
    ...
    let chain: Promise<void> = Promise.resolve();
    emojis.forEach((emoji) => { chain = chain.then(() => applyOne(channelId, msgId, emoji, failed)); });
    chain
        .then(() => delay(VERIFY_DELAY_MS))
        .then(() => verifyAndFix(channelId, msgId, emojis, 0, failed))
        .catch(() => {});
}
...
const S = StyleSheet.create({
    ...
    browseBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", borderRadius: 10, paddingVertical: 11, marginTop: 10 },
    browseTxt: { fontSize: 14, fontWeight: "700" },
    pickGuildRow: { flexDirection: "row", alignItems: "center", paddingVertical: 11, borderBottomWidth: 1 },
    pickGuildIcon: { width: 30, height: 30, borderRadius: 15, marginRight: 10 },
    pickGuildName: { flex: 1, fontSize: 15, fontWeight: "600" },
    pickGuildCount: { fontSize: 13, fontWeight: "600" },
...
function EmojiPicker({ onPick, onDone }: any) {
    const [query, setQuery] = useState("");
    const [expanded, setExpanded] = useState<Record<string, boolean>>({});
    const groups = useMemo(() => guildEmojiGroups(), []);
    const q = query.trim().toLowerCase();

    const emojiBtn = (e: any, key: any) => h(TouchableOpacity, {
        key, style: [S.pickEmoji, { backgroundColor: c("BACKGROUND_TERTIARY", "#1e1f22") }],
        onPress: () => onPick(`<${e.animated ? "a" : ""}:${e.name}:${e.id}>`),
    }, h(Image, { source: { uri: customEmojiUrl(e.id, e.animated) }, style: S.pickImg }));

    let body: any;
    if (q) {
        const matches: any[] = [];
        groups.forEach((g) => g.emojis.forEach((e: any) => {
            if (matches.length < 300 && String(e.name).toLowerCase().indexOf(q) >= 0) matches.push(e);
        }));
        body = matches.length
            ? h(View, { style: S.pickGrid }, matches.map((e, i) => emojiBtn(e, i)))
            : h(Text, { style: [S.noEmoji, { color: c("TEXT_MUTED", "#949ba4") }] }, "No emojis match");
    } else {
        body = groups.map((g) => h(View, { key: g.id },
            h(TouchableOpacity, {
                style: [S.pickGuildRow, { borderBottomColor: c("BORDER_FAINT", "#ffffff14") }],
                onPress: () => setExpanded((x: any) => ({ ...x, [g.id]: !x[g.id] })),
            },
                g.icon
                    ? h(Image, { source: { uri: `https://cdn.discordapp.com/icons/${g.id}/${g.icon}.${String(g.icon).indexOf("a_") === 0 ? "gif" : "png"}?size=32` }, style: S.pickGuildIcon })
                ...
                h(Text, { style: [S.pickGuildCount, { color: c("TEXT_MUTED", "#949ba4") }] }, `${g.emojis.length}  ${expanded[g.id] ? "⌃" : "⌄"}`),
            ),
            expanded[g.id] ? h(View, { style: S.pickGrid }, g.emojis.map((e: any, i: number) => emojiBtn(e, i))) : null,
        ));
    }
    ...
    return h(ScrollView, { style: [S.container, { backgroundColor: c("BACKGROUND_PRIMARY", "#313338") }], contentContainerStyle: S.content },
        ...
        h(Text, { style: [S.title, { color: c("HEADER_PRIMARY", "#fff"), fontSize: 19 }] }, "Server emojis"),
        h(Text, { style: [S.subtitle, { color: c("TEXT_MUTED", "#949ba4") }] },
            groups.length ? "Tap a server to expand, then tap an emoji to add it." : "No servers with custom emojis found."),
        h(TextInput, {
            ...
            value: query, onChangeText: setQuery, placeholder: "Search emojis by name", placeholderTextColor: c("TEXT_MUTED", "#87898c"),
        }),
        body,
    );
}
...
function Settings() {
    ...
    if (picking) {
        const append = (token: string) => {
            const add = (v: string) => (v.trim() ? v.trim() + " " : "") + token;
            if (picking === "edit") setEditInput(add);
            else setNewEmojis(add);
        };
        return h(EmojiPicker, { onPick: append, onDone: () => setPicking(null) });
    }
    ...
    if (editTarget && getUsers()[editTarget]) {
        ...
        return h(ScrollView, { style: [S.container, { backgroundColor: c("BACKGROUND_PRIMARY", "#313338") }], contentContainerStyle: S.content },
            ...
            h(Text, { style: [S.hint, { color: c("TEXT_MUTED", "#949ba4") }] },
                "Separate with spaces. Server emojis: type :name: or use the picker below (Nitro needed for other servers)."),
            h(TouchableOpacity, { style: [S.browseBtn, { backgroundColor: c("BACKGROUND_SECONDARY", "#2b2d31") }], onPress: () => setPicking("edit") },
